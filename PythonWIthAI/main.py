...
# opencv-python : 이미지 및 비디오 처리, 컴퓨터 비전 기능(robotflow 대체)
...
import numpy as np      # 배열 및 연산을 위한 넘파이 라이브러리
from starlette.middleware.base import BaseHTTPMiddleware
from ultralytics import YOLO    # yolo8 모델 사용 울트라리틱스        https://docs.ultralytics.com/ko/usage/python/#using-trainers
...
class DetectionResult(BaseModel):   # pydantic을 사용하여 데이터 모델을 정의 (응답 데이터를 구조화)
    message : str                   # 클라이언트가 보낸 메시지
    image : str                     # Base64로 인코딩된 탐지 결과 이미지
    
# 객체 탐지 함수
# 모델에 이미지를 넣어 객체를 탐지하고
# 그 결과 바운딩 박스 정보를 추출한 후 이미지에 바운딩 박스와 클래스 이름, 신뢰도를 표시한 후 반환
def detect_objects(image: Image):
    ...
    result_image = Image.fromarray(img) # 결과 이미지를 PIL 로 반환
    return result_image
    # 결론 : YOLO 모델로 객체 탐지 수행
    # 탐지된 객체에 대해 바운딩 박스를 그리고 정확도 정수를 이미지에 표시
    # 결과 이미지를 PIL 이미지로 변환하여 반환

@app.get("/")
async def index():
    return {"message" : "Hello FastAPI 8000"}

@app.post("/detect", response_model=DetectionResult)
async def detect_service(message: str = Form(...), file: UploadFile = File(...)):
    image = Image.open(io.BytesIO(await file.read()))   # 이미지를 읽어서 PIL 이미지로 변환
    ...
    img_str = base64.b64encode(buffered.getvalue()).decode("utf-8") # 이미지 결과를 base64로 인코딩
    
    return DetectionResult(message=message, image=img_str)  # 이 json 리턴값을 스프링부트에 보내줄 것
    # 결론 : http://localhost:8001/detect 경로에 post 요청 처리
    # 클라이언트가 업로드한 이미지를 읽고 PIL 이미지로 변환 후, RGB 로 convert
    # https://developer.mozilla.org/ko/docs/Glossary/Alpha 알파채널에 대한 설명(대충 불투명도란 이야기)
